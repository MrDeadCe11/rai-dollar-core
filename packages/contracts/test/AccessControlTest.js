...
contract('Access Control: Liquity functions with the caller restricted to Liquity contract(s)', async accounts => {
  ...
  before(async () => {
    ...
    collateralToken = coreContracts.collateralToken
  ...
  describe('BorrowerOperations', async accounts => { 
    it("moveETHGainToTrove(): reverts when called by an account that is not StabilityPool", async () => {
      const collToAdd = toBN(dec(100, 18))
      // Attempt call from alice
      try {
        const tx1= await borrowerOperations.moveCollateralGainToTrove(bob, collToAdd, bob, bob, { from: bob })
        console.log(tx1)
      } catch (err) {
  ...
  describe('TroveManager', async accounts => {
    ...
    it("applyPendingRewards(): reverts when called by an account that is not BorrowerOperations", async () => {
      // Attempt call from alice
      try {
        const txAlice = await rewards.applyPendingRewards(bob, { from: alice })
    ...
    it("updateRewardSnapshots(): reverts when called by an account that is not BorrowerOperations", async () => {
      // Attempt call from alice
      try {
        const txAlice = await rewards.updateTroveRewardSnapshots(bob, { from: alice })
    ...
    it("removeStake(): reverts when called by an account that is not BorrowerOperations", async () => {
      // Attempt call from alice
      try {
        const txAlice = await rewards.removeStake(bob, { from: alice })
    ...
    it("updateStakeAndTotalStakes(): reverts when called by an account that is not BorrowerOperations", async () => {
      // Attempt call from alice
      try {
        const txAlice = await rewards.updateStakeAndTotalStakes(bob, { from: alice })
  ...
  describe('ActivePool', async accounts => {
    ...
    // fallback (payment)	TODO: do we need fallback to revert if anyone sends eth to the activePool?
  ...
  describe('DefaultPool', async accounts => {
    ...
    it("sendCollateralToActivePool(): reverts when called by an account that is not TroveManager", async () => {
      ...
      try {
        ...
        assert.include(err.message, "revert")
        assert.include(err.message, "DefaultPool: Caller is not Rewards")
      }
    })
    ...
    it("increaseLUSDDebt(): reverts when called by an account that is not TroveManager", async () => {
      ...
      try {
        ...
        assert.include(err.message, "revert")
        assert.include(err.message, "Caller is not Rewards")
      }
    })
    ...
    // fallback (payment)	TODO: Skipped.  sending eth to active pool does not change collateral balance
  ...
  describe('StabilityPool', async accounts => {
    ...
    it("offset(): reverts when called by an account that is not TroveManager", async () => {
      // Attempt call from alice
      try {
        txAlice = await stabilityPool.offset(100, 100, 10, 100, 100, 10, { from: alice })
        assert.fail(txAlice)
      } catch (err) {
        assert.include(err.message, "revert")
        assert.include(err.message, "Caller is not Liq")
      }
    })
