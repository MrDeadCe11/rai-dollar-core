...
contract('StabilityPool - LQTY Rewards', async accounts => {
  ...
  describe("LQTY Rewards", async () => {
    ...
    beforeEach(async () => {
      ...
      stabilityPoolInterface = (await ethers.getContractAt("StabilityPool", contracts.stabilityPool.address)).interface;
    ...
    it("withdrawFromSP(): reward term G does not update when no LQTY is issued", async () => {
      ...
      await borrowerOperations.openTrove(dec(10000, 'ether'), dec(10000, 18), whale, whale, false, { from: whale })
      await borrowerOperations.openTrove(dec(1000, 'ether'), dec(10000, 18), A, A, false, { from: A })
      await stabilityPool.provideToSP(dec(10000, 18), ZERO_ADDRESS, { from: A })
    ...
    it("withdrawFromSP(): Depositors with equal initial deposit withdraw correct LQTY gain. No liquidations. No front end.", async () => {
      ...
      await borrowerOperations.openTrove(dec(100, 'ether'), dec(1, 22), A, A, false, { from: A })
      await borrowerOperations.openTrove(dec(100, 'ether'), dec(1, 22), B, B, false, { from: B })
      await borrowerOperations.openTrove(dec(100, 'ether'), dec(1, 22), C, C, false, { from: C })
      await borrowerOperations.openTrove(dec(100, 'ether'), dec(1, 22), D, D, false, { from: D })
      ...
      const expectedLQTYGain_1yr = distributable.div(toBN('3'));
    ...
    it("withdrawFromSP(): Depositors with varying initial deposit withdraw correct LQTY gain. No liquidations. No front end. #1", async () => {
      const initialIssuance = await communityIssuanceTester.totalLQTYIssued()
      assert.equal(initialIssuance, 0)

      // Whale opens Trove with 10k ETH
      await borrowerOperations.openTrove(dec(10000, 'ether'), await getOpenTroveLUSDAmount(dec(10000, 18)), whale, whale, false, { from: whale })

      await borrowerOperations.openTrove(dec(200, 'ether'), dec(10000, 18), A, A, false, { from: A })
      await borrowerOperations.openTrove(dec(300, 'ether'), dec(20000, 18), B, B, false, { from: B })
      await borrowerOperations.openTrove(dec(400, 'ether'), dec(30000, 18), C, C, false, { from: C })
      await borrowerOperations.openTrove(dec(100, 'ether'), dec(10000, 18), D, D, false, { from: D })
      ...
      const B_expectedLQTYGain_1yr = distributable
        // .div(toBN('2')) // 50% of total issued after 1 year
      ...
      const B_expectedLQTYGain_2yr = distributable2
        // .mul(toBN('3')).div(toBN('4')) // 75% of total issued after 1 year
        .div(toBN('3'))  // B gets 2/6 = 1/3 of the issuance
    ...
    it("withdrawFromSP(): Depositors with varying initial deposit withdraw correct LQTY gain. No liquidations. No front end. #2", async () => {
      const initialIssuance = await communityIssuanceTester.totalLQTYIssued()
      assert.equal(initialIssuance, 0)
      ...
      await borrowerOperations.openTrove(dec(200, 'ether'), dec(10000, 18), A, A, false, { from: A })
      await borrowerOperations.openTrove(dec(300, 'ether'), dec(20000, 18), B, B, false, { from: B })
      await borrowerOperations.openTrove(dec(300, 'ether'), dec(30000, 18), C, C, false, { from: C })
      await borrowerOperations.openTrove(dec(400, 'ether'), dec(40000, 18), D, D, false, { from: D })
      await borrowerOperations.openTrove(dec(400, 'ether'), dec(40000, 18), E, E, false, { from: E })

      await borrowerOperations.openTrove(dec(300, 'ether'), await getOpenTroveLUSDAmount(dec(30000, 18)), defaulter_1, defaulter_1, false, { from: defaulter_1 })
      ...
      // consider drip
      assert.isAtMost(getDifference(await stabilityPool.getTotalLUSDDeposits(), expSpDeposits), 1000)
      ...
      const B_expectedLQTYGain_Y1 = distributable
        // .div(toBN('2')) // 50% of total issued in Y1
        .div(toBN('3'))  // B gets 2/6 = 1/3 of the issuance
      ...
      BDeposit = await stabilityPool.getCompoundedLUSDDeposit(B)
      CDeposit = await stabilityPool.getCompoundedLUSDDeposit(C)
      DDeposit = await stabilityPool.getCompoundedLUSDDeposit(D)

      totalDeposit = await stabilityPool.getTotalLUSDDeposits()

      A_denom = totalDeposit.mul(toBN(dec(1,18))).div(ADeposit)
      B_denom = totalDeposit.mul(toBN(dec(1,18))).div(BDeposit)
      C_denom = totalDeposit.mul(toBN(dec(1,18))).div(CDeposit)
      D_denom = totalDeposit.mul(toBN(dec(1,18))).div(DDeposit)

      // E deposits and withdraws, creating LQTY issuance
      await stabilityPool.provideToSP(dec(1, 18), ZERO_ADDRESS, { from: E })
      // ICR is too low to withdraw any amount, however 0 will
      await stabilityPool.withdrawFromSP(0, { from: E })
     ...
     // console.log("A_expectedLQTYGain_Y2", A_expectedLQTYGain_Y2.toString())
      ...
      // Each depositor withdraws 0 to trigger final issuance, all troves are ICR < MCR so no actual withdrawal is permitted
      await stabilityPool.withdrawFromSP(0, { from: A })
      await stabilityPool.withdrawFromSP(0, { from: B })
      await stabilityPool.withdrawFromSP(0, { from: C })
      await stabilityPool.withdrawFromSP(0, { from: D })
    ...
    it('withdrawFromSP(): Depositor withdraws correct LQTY gain after serial pool-emptying liquidations. No front-ends.', async () => {
      ...
      // A, B each borrow 10k LUSD
      const depositors_1 = [A, B]
      for (account of depositors_1) {
        await borrowerOperations.openTrove(dec(200, 'ether'), dec(10000, 18), account, account, false, { from: account })
      }

      // C, D each borrow 10k LUSD
      const depositors_2 = [C, D]
      for (account of depositors_2) {
        await borrowerOperations.openTrove(dec(200, 'ether'), dec(10000, 18), account, account, false, { from: account })
      }

      // Erin, Flyn each borrow 100 LUSD
      const depositors_3 = [E, F]
      for (account of depositors_3) {
        await borrowerOperations.openTrove(dec(200, 'ether'), dec(10000, 18), account, account, false, { from: account })
      }

      // Graham, Harriet each borrow 10k LUSD
     ...
     // console.log("totalIssuanceAfterWithdrawal", totalIssuanceAfterWithdrawal.toString())
      ...
      // price temporarily up to allow withdrawals
      await priceFeed.setPrice(dec(200, 18));
      for (const depositor of depositors_2) {
        await stabilityPool.withdrawFromSP(dec(10000, 18), { from: depositor })
      }
      await priceFeed.setPrice(dec(100, 18));
     ...
     // console.log("totalIssuanceDiff_M1", totalIssuanceDiff_M1.toString())
      ...
      const expectedLQTYGain_M1 = totalIssuanceDiff_M1.div(th.toBN('2'))
    ...
    it('LQTY issuance for a given period is obtainable as the SP cannot be emptied during the period', async () => {
      ...
      await borrowerOperations.openTrove(dec(200, 'ether'), dec(16000, 18), A, A, false, { from: A })
      await borrowerOperations.openTrove(dec(100, 'ether'), dec(10000, 18), B, B, false, { from: B })
      await borrowerOperations.openTrove(dec(200, 'ether'), dec(16000, 18), C, C, false, { from: C })
      ...
      // C withdraws
      await stabilityPool.withdrawFromSP(dec(10000, 18), { from: C })
      const whaleGainC2 = await stabilityPool.getDepositorLQTYGain(whale)
      ...
      assert.isAtMost(getDifference(C_LQTYGain.add(whaleGainC2).sub(whaleGainC1), issuance_M4), 1e15)
      ...
      assert.isAtMost(getDifference(CIBalanceDifference.add(whaleGainC2).sub(whaleGainC1), expectedLQTYSentOutFromCI), 1e15)
    })
    ...
    A make deposit 100k LUSD
    1 month passes. L1 decreases P: P = 1e-5 P. L1:   99999 LUSD, 1000 ETH
    B makes deposit 99999
    1 month passes. L2 decreases P: P =  1e-5 P. L2:  99999 LUSD, 1000 ETH
    C makes deposit  99999
    1 month passes. L3 decreases P: P = 1e-5 P. L3:  99999 LUSD, 1000 ETH
    D makes deposit  99999
    1 month passes. L4 decreases P: P = 1e-5 P. L4:  99999 LUSD, 1000 ETH
    E makes deposit  99999
    1 month passes. L5 decreases P: P = 1e-5 P. L5:  99999 LUSD, 1000 ETH
    =========
    F makes deposit 100
    1 month passes. L6 empties the Pool. L6:  10000 LUSD, 1000 ETH
    ...
    it("withdrawFromSP(): Several deposits of 100k LUSD span one scale factor change. Depositors withdraw correct LQTY gains", async () => {
      ...
      await borrowerOperations.openTrove(dec(100, 'ether'), await getOpenTroveLUSDAmount(dec(10000, 18)), whale, whale, false, { from: whale })

      const fiveDefaulters = [defaulter_1, defaulter_2, defaulter_3, defaulter_4, defaulter_5]

      await borrowerOperations.openTrove(dec(100000, 'ether'), dec(100000, 18), ZERO_ADDRESS, ZERO_ADDRESS, false, { from: A })
      await borrowerOperations.openTrove(dec(100000, 'ether'), dec(100000, 18), ZERO_ADDRESS, ZERO_ADDRESS, false, { from: B })
      await borrowerOperations.openTrove(dec(100000, 'ether'), dec(100000, 18), ZERO_ADDRESS, ZERO_ADDRESS, false, { from: C })
      await borrowerOperations.openTrove(dec(100000, 'ether'), dec(100000, 18), ZERO_ADDRESS, ZERO_ADDRESS, false, { from: D })
      await borrowerOperations.openTrove(dec(100000, 'ether'), dec(100000, 18), ZERO_ADDRESS, ZERO_ADDRESS, false, { from: E })
      await borrowerOperations.openTrove(dec(100000, 'ether'), dec(100000, 18), ZERO_ADDRESS, ZERO_ADDRESS, false, { from: F })

      for (const defaulter of fiveDefaulters) {
        // Defaulters 1-5 each withdraw to 99999 debt (including gas comp)
        //await borrowerOperations.openTrove(await getOpenTroveLUSDAmount('99999000000000000000000'), defaulter, defaulter, false, { from: defaulter, value: dec(1000, 'ether') })

        await borrowerOperations.openTrove(dec(1000, 'ether'), await getOpenTroveLUSDAmount(dec(99999, 18)), defaulter, defaulter, false, { from: defaulter })
      }

      // Defaulter 6 withdraws to 10k debt (inc. gas comp)
      await borrowerOperations.openTrove(dec(1000, 'ether'), await getOpenTroveLUSDAmount(dec(100000, 18)), defaulter_6, defaulter_6, false, { from: defaulter_6 })
      ...
      // A provides to SP
      await stabilityPool.provideToSP(dec(100000, 18), ZERO_ADDRESS, { from: A })
     ...
     // console.log("expP1", expP1.toString())
      ...
      // B provides to SP
      await stabilityPool.provideToSP(dec(99999, 18), ZERO_ADDRESS, { from: B })
      const totalIssuanceAfterWithdrawal = await communityIssuanceTester.totalLQTYIssued()
      // large time passes increasing interest enough to avoid P scale change
      // 1 month passes
      await th.fastForwardTime(timeValues.SECONDS_IN_ONE_MONTH, web3.currentProvider)
      ...
      // Since scale has changed, th.getNewPAfterLiquidation() output needs to be divided by 1e9
      ...
      // D provides to SP
      await stabilityPool.provideToSP(dec(99999, 18), ZERO_ADDRESS, { from: D })
     ...
     // console.log("expP4", expP4.toString())
      ...
      assert.isTrue(P6.div(toBN(dec(1,9))).eq(expP6))
      //assert.equal(await stabilityPool.P(), dec(1, 15)) // Scale changes and P changes:: P = 1e(11-5+9) = 1e15
      ...
      // since the test setup takes longer than 60 seconds we have to subtract the issuance diff from issuance 1 since there will be extra unexpected issuance
      const totalIssuanceDiff = totalIssuanceAfterWithdrawal.sub(issuanceBeforeWithdrawal)
     ...
     // console.log("LQTYGain_A", LQTYGain_A.toString())
     ...
     to the liquidation that mostly depleted it.  Error tolerance = 1e-3 tokens. */
      ...
      const expectedGainA = totalIssuanceDiff.add(issuance_M2.div(toBN('100000')))
    ...
    it("withdrawFromSP(): Depositors with equal initial deposit withdraw correct LQTY gain. No liquidations. Front ends and kickback rates.", async () => {
      ...
      await borrowerOperations.openTrove(dec(100, 'ether'), dec(10000, 18), A, A, false, { from: A })
      await borrowerOperations.openTrove(dec(100, 'ether'), dec(10000, 18), B, B, false, { from: B })
      await borrowerOperations.openTrove(dec(100, 'ether'), dec(10000, 18), C, C, false, { from: C })
      await borrowerOperations.openTrove(dec(100, 'ether'), dec(10000, 18), D, D, false, { from: D })
      await borrowerOperations.openTrove(dec(100, 'ether'), dec(10000, 18), E, E, false, { from: E })
      ...
      const totalIssuanceAfterWithdrawal = await communityIssuanceTester.totalLQTYIssued()
      // since we are passing 60 seconds in the setup the inital issuance must be subtracted from the total issuance
      const expectedIssuance_Y1 = totalIssuanceAfterWithdrawal.sub(issuanceBeforeWithdrawal)
    ...
    it("withdrawFromSP(): Depositors with varying initial deposit withdraw correct LQTY gain. Front ends and kickback rates", async () => {
      ...
      await borrowerOperations.openTrove(dec(200, 'ether'), dec(10000, 18), A, A, false, { from: A })
      await borrowerOperations.openTrove(dec(800, 'ether'), dec(60000, 18), B, B, false, { from: B })
      await borrowerOperations.openTrove(dec(400, 'ether'), dec(30000, 18), C, C, false, { from: C })
      await borrowerOperations.openTrove(dec(500, 'ether'), dec(40000, 18), D, D, false, { from: D })

      await borrowerOperations.openTrove(dec(400, 'ether'), dec(30000, 18), E, E, false, { from: E })

      // D1, D2, D3 open troves with total debt 50k, 30k, 10k respectively (inc. gas comp)
      await borrowerOperations.openTrove(dec(500, 'ether'), await getOpenTroveLUSDAmount(dec(50000, 18)), defaulter_1, defaulter_1, false, { from: defaulter_1 })
      await borrowerOperations.openTrove(dec(200, 'ether'), await getOpenTroveLUSDAmount(dec(20000, 18)), defaulter_2, defaulter_2, false, { from: defaulter_2 })
     ...
     // console.log("Status", (await troveManager.getTroveStatus(defaulter_1)).toString())
      ...
      const A_share_M1 = totalIssuanceDiff.mul(toBN('10000')).div(toBN('100000'))
      const A_expectedLQTYGain_M1 = F1_kickbackRate.mul(A_share_M1).div(toBN(dec(1, 18)))

      const B_share_M1 = totalIssuanceDiff.mul(toBN('20000')).div(toBN('100000'))
      const B_expectedLQTYGain_M1 = F2_kickbackRate.mul(B_share_M1).div(toBN(dec(1, 18)))

      const C_share_M1 = totalIssuanceDiff.mul(toBN('30000')).div(toBN('100000'))
      const C_expectedLQTYGain_M1 = F2_kickbackRate.mul(C_share_M1).div(toBN(dec(1, 18)))

      const D_share_M1 = totalIssuanceDiff.mul(toBN('40000')).div(toBN('100000'))
    ...
    it("withdrawFromSP(): Several deposits of 10k LUSD span one scale factor change. Depositors withdraw correct LQTY gains", async () => {
      ...
      for (const defaulter of _4_Defaulters) {
        // Defaulters 1-4 each withdraw to 99999 debt (including gas comp)
        await borrowerOperations.openTrove(dec(1000, 'ether'), await getOpenTroveLUSDAmount(dec(99999, 18)), defaulter, defaulter, false, { from: defaulter })
      }
      ...
      await borrowerOperations.openTrove(dec(200, 'ether'), dec(5000, 18), A, A, false, { from: A })
     ...
     // console.log("drip", drip.toString())
      ...
      const issuanceBeforeM1 = await communityIssuanceTester.totalLQTYIssued()

      // Defaulter 1 liquidated.  Value of P updated to  to 9999999, i.e. in decimal, ~1e-10
      const txL1 = await liquidations.liquidate(defaulter_1, { from: owner });

      const issuanceAfterM1 = await communityIssuanceTester.totalLQTYIssued()
      const totalIssuanceDiffM1 = issuanceAfterM1.sub(issuanceBeforeM1)
     ...
     // console.log("P1", P1.toString())
      ...
      // C provides to SP
      await borrowerOperations.openTrove(dec(2000, 'ether'), dec(99999, 18), C, C, false, { from: C })
      await stabilityPool.provideToSP(dec(99999, 18), frontEnd_1, { from: C })

      // 1 month passes (M2)
      await th.fastForwardTime(timeValues.SECONDS_IN_ONE_MONTH, web3.currentProvider)
      const issuanceBeforeM2 = await communityIssuanceTester.totalLQTYIssued()
      // Defaulter 2 liquidated
      const txL2 = await liquidations.liquidate(defaulter_2, { from: owner });
      const issuanceAfterM2 = await communityIssuanceTester.totalLQTYIssued()
      const totalIssuanceDiffM2 = issuanceAfterM2.sub(issuanceBeforeM2)
      assert.isFalse(await sortedTroves.contains(defaulter_2))
      assert.isTrue(txL2.receipt.status)
     ...
     // console.log("P2", P2.toString())
      ...
      // D provides to SP
      await borrowerOperations.openTrove(dec(2000, 'ether'), dec(99999, 18), D, D, false, { from: D })
      await stabilityPool.provideToSP(dec(99999, 18), frontEnd_1, { from: D })
      ...
      // E provides to SP
      await borrowerOperations.openTrove(dec(2000, 'ether'), dec(99999, 18), E, E, false, { from: E })
      await stabilityPool.provideToSP(dec(99999, 18), frontEnd_1, { from: E })
     ...
     */
      ...
      const expectedLQTYGain_A_and_B = 
        kickbackRate
          .mul(totalIssuanceDiffM1)
          .div(toBN('2'))
          .div(toBN(dec(1, 18))) // gain from L1
          .add(
            kickbackRate.mul(totalIssuanceDiffM2)
              .div(toBN('2'))
              .div(toBN(dec(1, 18)))
              .div(toBN('100000'))
      ...
      const issuance1st4Months = totalIssuanceDiffM1.add(totalIssuanceDiffM2).add(issuance_M3).add(issuance_M4)
